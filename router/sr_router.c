...
void send_icmp(struct sr_instance* sr,
    ...
    struct sr_ip_hdr *old_ip_hdr = (sr_ip_hdr_t*)(packet + sizeof(sr_ethernet_hdr_t));
  ...
  if(type == 3){
    ...
    /*Copy the header to ethernet packet*/
    memcpy(reply_eth->ether_shost, matching_if->addr, ETHER_ADDR_LEN);
    ...
    reply_ip->ip_hl = 5;
    reply_ip->ip_p = ip_protocol_icmp;
    reply_ip->ip_src = old_ip_hdr->ip_dst;
    reply_ip->ip_dst = old_ip_hdr->ip_src;
    reply_ip->ip_len = old_ip_hdr->ip_len;
    reply_ip->ip_ttl = 64;
    reply_ip->ip_sum = 0;
    reply_ip->ip_sum = cksum(reply_ip, sizeof(sr_ip_hdr_t));
